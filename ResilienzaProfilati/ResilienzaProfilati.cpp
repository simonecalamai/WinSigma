// ResilienzaProfilati.cpp : Defines the initialization routines for the DLL.
//
...
BEGIN_MESSAGE_MAP(CResilienzaProfilatiApp, CWinApp)
	//{{AFX_MSG_MAP(CResilienzaProfilatiApp)
...
/////////////////////////////////////////////////////////////////////////////
// CResilienzaProfilatiApp construction

CResilienzaProfilatiApp::CResilienzaProfilatiApp()
{
...
/////////////////////////////////////////////////////////////////////////////
// The one and only CResilienzaProfilatiApp object

CResilienzaProfilatiApp theApp;
...
__declspec( dllexport ) int LoadProveMateriale(CStringArray* stringhe, CAllTables* tabelle, CDWordArray* parySerieEliminate)
{
	...
	strTempMateriale.Format("%d %s", contaProvini, MATERIALE);
																				 ...
																				 CStringArray* pFieldValues, byte primoCampo, int numeroInterno)
{
  ...
  pFieldValues->RemoveAll();
	...
	switch(pDati->nVersione)
		...
		case 0:
			{
			pFieldNames->Add("temperatura");
			pFieldNames->Add("provetta");
			pFieldNames->Add("sezione");
			pFieldNames->Add("kv");
		
			pFieldValues->Add(pDati->temperatura);
			pDati->provetta == SOSTITUTIVA ? str = STR_SOSTITUTIVA : str = STR_NORMALE ;
			pFieldValues->Add(str);
			
			pFieldValues->Add(pDati->sezione);
			str.Format("%.0f",pDati->kv);
			pFieldValues->Add(str);
			}
			break;
		case 1:;
