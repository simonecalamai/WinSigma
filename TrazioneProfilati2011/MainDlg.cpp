...
void CMainDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_EDT_LARGHEZZA, m_EdtLarghezza);
	DDX_Control(pDX, IDC_EDT_SPESSORE, m_EdtSpessore);
	...
	DDX_Text(pDX, IDC_EDT_LARGHEZZA, m_lfLarghezza);
	DDX_Text(pDX, IDC_EDT_SPESSORE, m_lfSpessore);
...
BEGIN_MESSAGE_MAP(CMainDlg, CXDialog)
	...
	ON_EN_KILLFOCUS(IDC_EDT_SPESSORE, OnKillfocusEdtSpessore)
	ON_EN_KILLFOCUS(IDC_EDT_LARGHEZZA, OnKillfocusEdtLarghezza)
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()
...
void CMainDlg::DatiCertificato()
{
	...
	{
		if( !m_pOperatoriSet->IsFieldNull(&m_pOperatoriSet->m_Sperimentatore) && m_pOperatoriSet->m_Sperimentatore ) 
		{
...
void CMainDlg::DisableModify(byte disabilita)
{
	if( m_bDatiModificabili )
	{
		...
		m_EdtSpessore.SetReadOnly(disabilita);
	...
	else
	{
		...
		m_EdtSpessore.SetReadOnly(TRUE);
...
void CMainDlg::OnSelchangedTreeProvini(NMHDR* pNMHDR, LRESULT* pResult) 
{
	...
	switch(m_pDatiCorrenti->nVersione)
		...
		case 0:
			{
				m_lfLarghezza			= m_pDatiCorrenti->larghezza;
				m_lfSpessore			= m_pDatiCorrenti->spessore;
...
void CMainDlg::OnButtonConferma() 
{
	...
	if(UpdateData(TRUE))
	{
		...
		switch(m_pDatiCorrenti->nVersione)
		{
			...
			{
					m_pDatiCorrenti->larghezza		= m_lfLarghezza;
					m_pDatiCorrenti->spessore			= m_lfSpessore;
...
void CMainDlg::OnBtnModifica() 
{
	if( m_bDatiModificabili )
	{
		...
		m_EdtLarghezza.SetFocus();
		m_EdtLarghezza.SetSel(0, -1);
	}
	else 
	{
...
void CMainDlg::OnKillfocusEdtSpessore() 
{
	CalcolaSezione();
}

void CMainDlg::OnKillfocusEdtLarghezza() 
{
	CalcolaSezione();
}


void CMainDlg::CalcolaSezione()
{
	UpdateData(TRUE);

	/* calcolo della sezione */
	double area = m_lfLarghezza * m_lfSpessore;
	/* arrotondamento */
	int n_area = (int)area;
	if(area - n_area < 0.50)
		m_lfArea = n_area;
