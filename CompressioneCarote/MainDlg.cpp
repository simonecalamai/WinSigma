...
void CMainDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_CHECK_ALLEGATO, m_BtnAllegato);
	...
	DDX_Control(pDX, IDC_EDIT_LUNG, m_EdtLunghezza);
	DDX_Control(pDX, IDC_EDIT_DIAMETRO, m_EdtDiametro);
	...
	DDX_Text(pDX, IDC_EDIT_DIAMETRO, m_lfDiametro);
	DDX_Text(pDX, IDC_EDIT_LUNG, m_lfLunghezza);
	...
	DDX_Check(pDX, IDC_CHECK_ALLEGATO, m_bAllegato);
	DDX_Text(pDX, IDC_STC_DIM1, m_strDiametro);
	DDX_Text(pDX, IDC_STC_DIM2, m_strLunghezza);
	DDX_Text(pDX, IDC_STATIC_UMAREA, m_strUMArea);
	//}}AFX_DATA_MAP
}
...
BOOL CMainDlg::OnInitDialog() 
{
  ...
  m_pProviniSet->m_strSort = "Codice";
	...
	//-- Verifico se la serie appartiene ad un certificato già emesso
	...
	if(!m_pSerieSet->IsFieldNull(&m_pSerieSet->m_DataProva))
			...
			m_DataProva = *m_pUltimaDataProve;

	// Rilevo l'eventuale diametro dichiarato
	if(!m_pSerieSet->IsFieldNull(&m_pSerieSet->m_Dimensione1))
		m_lfDiametro = m_pSerieSet->m_Dimensione1;
  ...
  if(!m_pSerieSet->IsFieldNull(&m_pSerieSet->m_Checked))
    m_bAllegato = m_pSerieSet->m_Checked;
  else
    m_bAllegato = 0;
	...
	if (m_pSerieSet->m_TipoCertificato == 9)
  {
  ...
  {
    str.Format("%d - Bipiramidale",BIPIRAMIDALE);
	  index = m_ComboTipoRottura.AddString(str);
	  m_ComboTipoRottura.SetItemData( index, BIPIRAMIDALE);
	  str.Format("%d - Sfaldamento obliquo",SFALDAMENTO);
	  index = m_ComboTipoRottura.AddString(str);	
	  m_ComboTipoRottura.SetItemData( index, SFALDAMENTO);
	  str.Format("%d - Sgretolamento",SGRETOLAMENTO);
	  index = m_ComboTipoRottura.AddString(str);
	  m_ComboTipoRottura.SetItemData( index, SGRETOLAMENTO);
	  str.Format("%d - Altro",ALTRO);
	  index = m_ComboTipoRottura.AddString(str);
	  m_ComboTipoRottura.SetItemData( index, ALTRO);
  }
...
void CMainDlg::DisableModify(boolean disabilita)
{
	if( m_bDatiModificabili)
		{
		m_EdtDiametro.SetReadOnly(disabilita);
    ...
    m_BtnAllegato.EnableWindow(!disabilita);
		...
		{
		m_EdtDiametro.SetReadOnly(TRUE);
	...
	if(UpdateData(TRUE))//Solo se i dati sono stati validati
	{
		...
		switch(m_pDatiCorrenti->nVersione)
			...
			case 0://elaborazione dei dati per la versione 0
				{
					m_pDatiCorrenti->diametro			= m_lfDiametro;
					m_pDatiCorrenti->lunghezza		= m_lfLunghezza;
					m_pDatiCorrenti->massa				= m_lfMassa;
					m_pDatiCorrenti->caricoSpinta	= m_lfCaricoSp;

					int index = m_ComboTipoRottura.GetCurSel();
					m_pDatiCorrenti->tipoRottura	= (byte)m_ComboTipoRottura.GetItemData(index);
				};
...
void CMainDlg::OnBtnModifica() 
{
	if( m_bDatiModificabili )
		...
		m_EdtDiametro.SetFocus();
		m_EdtDiametro.SetSel(0, -1);
	}
	else 
		{
	...
	switch(m_pDatiCorrenti->nVersione)
        ...
        CalcolaDati();
				...
				for(int pos = 0; pos < m_ComboTipoRottura.GetCount(); pos++)
					if((byte)m_ComboTipoRottura.GetItemData(pos) == m_pDatiCorrenti->tipoRottura)
...
void CMainDlg::AggiornaDatiSerie()
{
	...
	try
	{
      ...
      m_pSerieSet->m_Checked = m_bAllegato;
...
void CMainDlg::OnOK() 
{
	...
	*m_pUltimaDataProve = m_DataProva;

	if(m_DataProva < m_DataPrelievo && 
...
void CMainDlg::DatiCertificato()
{
	...
	if( !m_pSerieSet->IsFieldNull(&m_pSerieSet->m_Sperimentatore) )
		...
		UpdateData(FALSE); //leggo dal dialogo
		}
	else
		{
...
BOOL CMainDlg::CalcolaDati()
{
	...
	if( (m_lfDiametro!=0) && (m_lfLunghezza!=0) )
	{
		double area					= ((m_lfDiametro * m_lfDiametro))/2. * PI_GRECO / 2.;
		double compressione(0); 
    if (m_pSerieSet->m_TipoCertificato == 9)  
      compressione = (m_lfCaricoSp / area) * 100 * 100;
    else
      compressione = (m_lfCaricoSp / area) * 100;
