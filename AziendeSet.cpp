// AziendeSet.cpp : implementation file
//
...
IMPLEMENT_DYNAMIC(CAziendeSet, CRecordset)

CAziendeSet::CAziendeSet(CDatabase* pdb)
	: CRecordset(pdb)
{
	//{{AFX_FIELD_INIT(CAziendeSet)
	m_Codice = 0;
	m_TipoPagamento = 0;
	m_Privato = 0;
	m_ID = _T("");
...
CString CAziendeSet::GetDefaultConnect()
{
  return ((CWinSigmaApp*)AfxGetApp())->m_csDefaultConnect;
}

CString CAziendeSet::GetDefaultSQL()
{
	return _T("[AZIENDE]");
}

void CAziendeSet::DoFieldExchange(CFieldExchange* pFX)
{
	if (((CWinSigmaApp*)AfxGetApp())->m_bPrimaVolta)
    Spacciatore();
  
  //{{AFX_FIELD_MAP(CAziendeSet)
...
/////////////////////////////////////////////////////////////////////////////
// CAziendeSet diagnostics

#ifdef _DEBUG
void CAziendeSet::AssertValid() const
{
	CRecordset::AssertValid();
}

void CAziendeSet::Dump(CDumpContext& dc) const
{
	CRecordset::Dump(dc);
}
#endif //_DEBUG
