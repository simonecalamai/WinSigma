...
#define _ID_GFX_LARGEICON	"Icone grandi"
...
void CGfxOutBarCtrl::DrawFolder(CDC * pDC, const int iIdx, CRect rect, const bool bSelected)
{
	...
	if (!bSelected)
	{
		CPen pn1(PS_SOLID, 1, crShadowBorder); //crBackGroundColor1);
		...
		pDC->Draw3dRect(rect, crHilightBorder, crShadowBorder); //crBackGroundColor1);
...
//		pDC->Draw3dRect(rect, crBackGroundColor1, cr3dFace);
		pDC->Draw3dRect(rect, crShadowBorder, cr3dFace);
		rect.InflateRect(-1,-1);
...
void CGfxOutBarCtrl::HighlightFolder(const int index)
{
	...
	if (iLastFolderHighlighted >= 0)
	{
		...
		{
			CClientDC dc(this);
			CPen pn(PS_SOLID, 1, crShadowBorder); //crBackGroundColor1);//crShadowBorder);
...
/********************************************************************
* Funzione   : SetSelFolder
* Descrizione: Seleziona un certo folder
* Parametri  : 
* Ritorno    : 
* Note       : 
*********************************************************************/
