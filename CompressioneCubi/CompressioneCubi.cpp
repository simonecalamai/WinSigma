// CompressioneCubi.cpp : Defines the initialization routines for the DLL.
//

#include "stdafx.h"
#include "CompressioneCubi.h"
#include "MainDlg.h"
...
/////////////////////////////////////////////////////////////////////////////
// CCompressioneCubiApp

BEGIN_MESSAGE_MAP(CCompressioneCubiApp, CWinApp)
	//{{AFX_MSG_MAP(CCompressioneCubiApp)
...
/////////////////////////////////////////////////////////////////////////////
// CCompressioneCubiApp construction

CCompressioneCubiApp::CCompressioneCubiApp()
{
...
/////////////////////////////////////////////////////////////////////////////
// The one and only CCompressioneCubiApp object

CCompressioneCubiApp theApp;
...
__declspec( dllexport ) int OpenDlg(long codSerie, CAllTables* tabelle, long* ultimoSperimentatore, CTime* pUltimaDataProva)
{
  ...
  if (pSerieSet->m_TipoCertificato == 8) 
    {
      tabelle->m_pTipiCertificatoSet->m_strFilter = "Codice = 8";
...

{
  ...
  if (pTabelle->m_pSerieProviniSet->m_TipoCertificato == 8)
    rottura	= (pDati->caricoSp / area)*10 * 100;
  else
    rottura	= (pDati->caricoSp / area)*10;
...
//--- Gestione dei campi da scrivere unicamente per il primo provino ---//
	...
	if( primoCampo )
		  ...
		  else 
      {
        ...
        {
          CString s("");
          if (pTabelle->m_pSerieSet->m_TipoCertificato == 8)
              s = "> 28 gg";
          else
             s = "- n.d. -";
          pFieldValues->Add(s);
	...
	switch(pDati->nVersione)
		...
		case 0:
			...
			if (pTabelle->m_pSerieProviniSet->m_TipoCertificato != 12)
			{
      ...
      {
        //Gestione Norma UNI EN 12390-3 2003
        str.Format("%3.0f",pDati->lung);
			  pFieldValues->Add(str);
      	str.Format("%3.0f",pDati->larg);
			  pFieldValues->Add(str);
      	str.Format("%3.0f",pDati->alt);
			  pFieldValues->Add(str);
        str.Format("%4.0f",area);
        str = str.Mid(0,str.GetLength()-1);
        str += "0";
			  pFieldValues->Add(str);
      }
			else if (pTabelle->m_pSerieProviniSet->m_TipoCertificato == 12) 
      {
      ...
      else
      {
        ...
        str.Format("%.1f",pDati->lung);
			  pFieldValues->Add(str);
        str.Format("%.1f",pDati->larg);
			  pFieldValues->Add(str);
        str.Format("%.1f",pDati->alt);
        pFieldValues->Add(str);
        str.Format("%4.1f",area);
			  pFieldValues->Add(str);
      }

      if ((pTabelle->m_pSerieProviniSet->m_TipoCertificato == 8) ||
					(pTabelle->m_pSerieProviniSet->m_TipoCertificato == 12))
      {
        //Gestione Norma UNI EN 12390-3 2003
        CString App("");
