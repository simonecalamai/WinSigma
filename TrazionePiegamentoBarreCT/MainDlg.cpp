...
void CMainDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_EDIT_FORNITORE, m_ctrlFornitore);
	...
	DDX_Text(pDX, IDC_EDIT_FORNITORE, m_csFornitore);
	//}}AFX_DATA_MAP
}
...
void CMainDlg::OnBtnModifica() 
{
	if( m_bDatiModificabili )
	{
		...
		m_ctrlRotoliCT.EnableWindow(TRUE);
...
byte CMainDlg::CalcolaDati(BOOL bConDecimali)
{
	...
	if(m_lfLunghezza!=0)
	{
		...
		//Gestione dei decimali in base alla sezione rilecata
     if(!bConDecimali)
     {
  		if(sezRilevata>100)
...
//		snervamentoRelativo	= m_lfSnervamento/sezRilevata * 1000;
...
//		tensioneRelativa	= m_lfRottura/sezRilevata * 1000;
    ...
    tensioneRelativa = atof(s);
		...
		/////////////////////////////////////////////////
		// Visualizzazione dei dati calcolati nel dialogo
    if(bConDecimali)
    {
  		if(sezRilevata>100)
   			sezRilevata = floor(sezRilevata + 0.5);
...
BOOL CMainDlg::OnInitDialog() 
{
  ...
  m_pProviniSet->m_strSort = "Codice";
	...
	if(!m_pTipoCertSet->IsEOF())
		m_nVariante = m_pTipoCertSet->m_Variante;
	else
		m_nVariante = 0;
