...
void CMainDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_EDT_TESNSIONE, m_EdtTensione);
	DDX_Control(pDX, IDC_EDT_ROTTURA, m_EdtRottura);
	DDX_Control(pDX, IDC_EDT_MODULO, m_edtModulo);
	DDX_Control(pDX, IDC_EDT_LIMITE2, m_EdtLimite2);
	DDX_Control(pDX, IDC_EDT_LIMITE1, m_EdtLimite1);
	...
	DDX_Text(pDX, IDC_EDT_LIMITE1, m_lfLimite1);
	DDX_Text(pDX, IDC_EDT_LIMITE2, m_lfLimite2);
	DDX_Text(pDX, IDC_EDT_MODULO, m_lfModulo);
	DDX_Text(pDX, IDC_EDT_ROTTURA, m_lfRottura);
	DDX_Text(pDX, IDC_EDT_TESNSIONE, m_lfTensione);
	DDX_Text(pDX, IDC_EDIT_DATA_VA, m_strDataAccettazione);
	DDX_Text(pDX, IDC_EDT_ALLUNGAMENTO, m_strAllungamento);
	//}}AFX_DATA_MAP
}
...
CByteArray* CMainDlg::InitBlob()
{
	...
	pDati->nVersione				= 0;
	pDati->allungamento[0]	= 0;
	pDati->area							= 0;
	pDati->limite1					= 0;
	pDati->limite2					= 0;
	pDati->moduloElastico		= 0;
	pDati->rottura					= 0;
	pDati->tensione					= 0;
...
void CMainDlg::DisableModify(byte disabilita)
{
	if( m_bDatiModificabili )
		...
		m_EdtLimite1.SetReadOnly(disabilita);
		m_EdtLimite2.SetReadOnly(disabilita);
		m_EdtTensione.SetReadOnly(disabilita);
		m_EdtRottura.SetReadOnly(disabilita);
		m_edtModulo.SetReadOnly(disabilita);
		...
		m_EdtLimite1.SetReadOnly(TRUE);
		m_EdtLimite2.SetReadOnly(TRUE);
		m_EdtTensione.SetReadOnly(TRUE);
		m_EdtRottura.SetReadOnly(TRUE);
		m_edtModulo.SetReadOnly(TRUE);
...
// Funzione necessaria per il corretto funzionamento 
// delle caratteristiche della classe CXDialog
...
// Controllo della coerenza sulla data scelta per le prove
	...
	switch(m_pDatiCorrenti->nVersione)
		...
		case 0:
			{			
				...
				m_lfLimite1				= m_pDatiCorrenti->limite1;
				m_lfLimite2				= m_pDatiCorrenti->limite2;
				m_lfTensione			= m_pDatiCorrenti->tensione;
				m_strAllungamento	= m_pDatiCorrenti->allungamento;
				m_lfRottura				= m_pDatiCorrenti->rottura;
				m_lfModulo				= m_pDatiCorrenti->moduloElastico;
			};
			...
			case 0://elaborazione dei dati per la versione 0
				{
					...
					m_pDatiCorrenti->limite1				= m_lfLimite1;
					m_pDatiCorrenti->limite2				= m_lfLimite2;	
					m_pDatiCorrenti->tensione				= m_lfTensione;
					m_pDatiCorrenti->rottura				= m_lfRottura;
					m_pDatiCorrenti->moduloElastico	= m_lfModulo;

					for(int i=0; i<min( (STR_ALLUNGAMENTO - 1), m_strAllungamento.GetLength()); i++)
						m_pDatiCorrenti->allungamento[i] = m_strAllungamento.GetAt(i);
					m_pDatiCorrenti->allungamento[i] = 0;
				};
			case 1 :; // predisposizione per l'introduzione di nuovi campi alla struttura dati
			}
		UpdateData(FALSE);
